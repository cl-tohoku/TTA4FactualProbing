...
def get_dec_fact_index(prompts_df):
    ...
    original_correct_fid = prompts_df.loc[(prompts_df['type'] == '0original') & (prompts_df['is_correct'] == 0.0)]['fact_id']
    n_corrects_by_fact = prompts_df.groupby(['fact_id']).sum().sort_values('is_correct', ascending=False)

    dec = n_corrects_by_fact.loc[n_corrects_by_fact.index.isin(original_correct_fid)]
    return dec.index.to_list()

def main():
    prompts_path = 'cache/v2.11d.4/prompts.csv'
    prompts_df = pd.read_csv(prompts_path)

    augmentation_path = 'cache/v2.11d.4/evaluate_all.pkl'
